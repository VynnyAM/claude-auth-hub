...
const Index = () => {
  ...
  // Notas analysis (Genogram specialist)
  ...
  const [blogEntries, setBlogEntries] = useState([
    ...
    },
    {
      id: 8,
      date: '24/10/2025',
      badge: { text: 'Atualização', color: 'blue' },
      title: 'Analisar Padrões e melhoria no modal de análise',
      description: 'Adicionado botão "Analisar padrões" em Notas e Brainstorming (usa a função Supabase "analyze-notes" com fallback local heurístico). O modal de análise agora abre sem escurecer toda a tela (overlay transparente) para não interromper o fluxo de trabalho.'
    },
    {
      id: 1,
      date: '22/10/2025',
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <Button 
              ...
              variant="outline" 
              size="sm"
              className={isTrialActive ? "bg-green-50 border-green-200 opacity-100" : ""}
            >
            ...
            <div className="bg-card rounded-xl shadow-md p-4">
              ...
              <div className="mt-3 flex items-center gap-2">
                <Button
                  ...
                  variant="outline"
                  onClick={async () => {
                    // Open analysis modal and run analysis
                    ...
                    try {
                      if (!notes || !notes.trim()) {
                        setAnalysisResult('Digite algumas notas para que eu possa analisar padrões.');
                        setAnalyzingNotes(false);
                        return;
                      }

                      // Try remote Supabase Function first
                      ...
                      if (error) {
                        ...
                        const local = (() => {
                          ...
                          if (lower.includes('alcool') || lower.includes('drog')) suggestions.push('Histórico de abuso de substâncias — investigar padrões de dependência multigeracional.');
                          if (lower.includes('divórc') || lower.includes('separ')) suggestions.push('Relações conjugais instáveis — verificar separações e impacto em relações parentais.');
                          if (lower.includes('adot') || lower.includes('filh') && lower.includes('fora')) suggestions.push('Possível adoção ou guarda fora da família biológica — sinalizar diferenças de linhagem.');
                          if (lower.includes('suic') || lower.includes('tentativ')) suggestions.push('Risco de autoagressão/suicídio em histórico familiar — considerar intervenção.');
                          if (suggestions.length === 0) return 'Nenhum padrão óbvio encontrado localmente. Você pode fornecer mais detalhes para uma análise mais profunda.';
                          return suggestions.join('\n- ');
                        })();

                        setAnalysisResult(local);
                      } else {
                        // If remote returned structured result, prefer it
                        // The function is expected to return { result: string }
                        const parsed = (data && (data as any).result) ? (data as any).result : JSON.stringify(data);
                        setAnalysisResult(parsed || 'Sem resultado da análise.');
                      }
                    } catch (err: any) {
                      setAnalysisResult('Erro ao analisar notas: ' + (err?.message || String(err)));
                    } finally {
                      setAnalyzingNotes(false);
                    }
                  }}
            ...
            <div className="bg-card rounded-xl shadow-md p-4 border-l-4 border-primary">
              ...
              <div className="space-y-4 max-h-96 overflow-y-auto">
                {blogEntries.map((entry) => (
                  ...
                  {/* Analysis modal for notes */}
                  <Dialog open={showAnalysisModal} onOpenChange={setShowAnalysisModal}>
                    <DialogContent overlayClassName="bg-transparent">
                      <DialogHeader>
                        <DialogTitle>Análise de Padrões (Especialista em Genograma)</DialogTitle>
                        <DialogDescription>
                          Resultado da análise das suas notas. Use como orientação clínica e investigativa — não substitui avaliação profissional.
                        </DialogDescription>
                      </DialogHeader>
                      <div className="mt-2">
                        {analyzingNotes ? (
                          <p>Analisando suas notas... (pode levar alguns segundos)</p>
                        ) : (
                          <div className="whitespace-pre-wrap text-sm">
                            {analysisResult || 'Nenhum resultado disponível.'}
                          </div>
                        )}
                      </div>
                      <DialogFooter>
                        <Button onClick={() => { if (analysisResult) navigator.clipboard?.writeText(analysisResult); }} variant="outline">Copiar resultado</Button>
                        <Button onClick={() => setShowAnalysisModal(false)}>Fechar</Button>
                      </DialogFooter>
                    </DialogContent>
                  </Dialog>
                    <p className="text-xs text-muted-foreground">
